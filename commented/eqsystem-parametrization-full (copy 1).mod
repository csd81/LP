...
# 'param' declares a parameter, and ':=' assigns it a
# fixed value. This section hard-codes all the data
# (coefficients and constants) into the model.
...
param c1x := 4; param c1y := -2;
param c1z :=  3; param c1w :=  0; param c1v :=  0;

# --- Coefficients for Equation 2 ---
param c2x := 0;
param c2y :=  0; param c2z := -1; param c2w := -1; param c2v :=  0;

# --- Coefficients for Equation 3 ---
param c3x := 0; param c3y :=  1; param c3z :=  1; param c3w :=  0;
param c3v :=  0;

# --- Coefficients for Equation 4 ---
param c4x := 4; param c4y := -7; param c4z :=  0;
param c4w :=  2; param c4v :=  5;

# --- Coefficients for Equation 5 ---
param c5x := 1; param c5y :=  0;
param c5z :=  2; param c5w :=  0; param c5v :=  3;

# === 3. Constraint Definitions ===
# "s.t." (subject to) introduces the equations.
# These equations are built using the variables (x, y, etc.)
# and the parameter values defined above.
s.t.
