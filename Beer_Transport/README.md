...
1.  **`Brewery_capacity_hl`**: Total beer transported *out* of any brewery cannot exceed its production capacity:
    $$\sum_{u \in \text{University}} \text{transporthl}[b,u] \le \text{productionhl}[b]$$
...
$$\sum_{b \in \text{Brewery}, u \in \text{University}} \text{transportationcostHUFperkgperkm} \times \text{distancekm}[b,u] \times \text{TotalMass}[b,u]$$
