...
s.t. Material_balance_for_intermediate_node
	{n in Nodes : n != StartNode && n != EndNode}:
	sum {a in Nodes} flow[a,n] = 0;

s.t. Material_balance_for_start_node:
	sum {a in Nodes} flow[a,StartNode] = -throughput;

s.t. Material_balance_for_end_node:
	sum {a in Nodes} flow[a,EndNode] = +throughput;
