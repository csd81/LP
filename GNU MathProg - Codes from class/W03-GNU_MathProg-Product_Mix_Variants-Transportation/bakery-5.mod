# (not a problem now, bot could be, easy with a bound: Cake #4: must be at least 10.)

# Cake #4: must be at least 25% of the total number of cakes produces
...
s.t. Min_ratio_of_Cake_4:
	(x1 + x2 + x3 + x4 + x5) * 0.25 <= x4;
