...
# these pairs must NOT be chosen
set Assignments_0 within Workers cross Jobs;

# these pairs must be chosen:
set Assignments_1 within Workers cross Jobs;
...
s.t. Assignments_0_Constraint {(w,j) in Assignments_0}:
	a[w,j] = 0;

s.t. Assignments_1_Constraint {(w,j) in Assignments_1}:
	a[w,j] = 1;
