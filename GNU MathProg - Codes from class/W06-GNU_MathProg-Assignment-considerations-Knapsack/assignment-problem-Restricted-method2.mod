...
param Preset_Choice {w in Workers, j in Jobs},
	in {0, 1, -1}, default -1;
	# 0: assignment NOT allowed
	# 1: assignment must be done
...
s.t. Preset_Constraints
	{w in Workers, j in Jobs :
		Preset_Choice[w,j] in {0,1} }:
	a[w,j] = Preset_Choice[w,j];
