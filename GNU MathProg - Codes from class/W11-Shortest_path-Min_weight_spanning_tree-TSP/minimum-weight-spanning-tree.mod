...
# -> Minimum weight "connected(!)" spanning subgraph problem

# flows with use variable, which must be binary
...
s.t. Use_constraint {a in Nodes, b in Nodes}:
	...
	# M=(card(Nodes)-1) (smallest possible, tends to be solved faster)
	flow[a,b] <= (card(Nodes)-1) * use[a,b];
...
s.t. Material_balance {n in Nodes}:
	sum {a in Nodes} flow[a,n] =
		if (n==CentralNode) then (-(card(Nodes)-1)) else
		(+1); # other (non-central) nodes
