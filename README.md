...
| \*\*Car Dealer Trading\*\* (`car\_dealer\_nicer.m`) | Integer (`buysell\[d,c]`), Continuous (`balance\[d]`, `garage\[d,c]`) | \*\*MILP/IP\*\* |

| \*\*Package Delivery\*\* (AMPL Model with `MaxItemsConstraint`) | Binary (`chose\[s,r]`, $\\text{deliver}\[s,o]$) | \*\*MILP/IP\*\* |

| \*\*Festival Set Covering\*\* | Binary ($y\_f$) | \*\*MILP/IP\*\* |
...
| \*\*Puska Assignment\*\* | Binary ($y\_{XY}$) | \*\*MILP/IP\*\* |
...
| \*\*Exam Scheduling (GLPK/AMPL)\*\* (`model\_2.mod`) | Binary ($\\text{atmegyek}$), Continuous ($\\text{tanulok}$) | \*\*MILP\*\* |
